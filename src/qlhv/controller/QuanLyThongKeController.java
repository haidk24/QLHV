...
public class QuanLyThongKeController {
    ...
    public void setDataToChart1(JPanel jpnItem) {
     ...
     public void setDataToChart2(JPanel jpnItem) {
        ...
        org.jfree.data.gantt.Task task; // Fully qualified name to avoid conflict

        if (listItem != null) {
            for (KhoaHocBean item : listItem) {
                taskSeries = new TaskSeries(item.getTen_khoa_hoc());
                task = new org.jfree.data.gantt.Task(item.getTen_khoa_hoc(), 
                    new SimpleTimePeriod(item.getNgay_bat_dau(), item.getNgay_ket_thuc()));
                taskSeries.add(task);
                ds.add(taskSeries);
            }
        }
